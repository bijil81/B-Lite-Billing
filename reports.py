...
class ReportsFrame(tk.Frame):
    ...
    def _build_list_tab(self):
        ...
        self._report_filter_buttons = {}
        self._report_filter_button_colors = {}
        for txt, col, mode, cmd in [
            ("Filter",     C["teal"],   "filter", lambda: (self._set_report_filter_mode("filter"), self._reset_report_pagination(), self._load())),
            ("Today",      C["blue"],   "today", lambda: (self._reset_report_pagination(), self._set_today())),
            ("This Month", C["purple"], "month", lambda: (self._reset_report_pagination(), self._set_month())),
            ("All",        "#636e72",   "all", lambda: (self._reset_report_pagination(), self._set_all())),
        ]:
            ...
            btn = ModernButton(ff, text=txt, image=btn_icon, compound="left", command=cmd,
            ...
            self._report_filter_buttons[mode] = btn
            self._report_filter_button_colors[mode] = col
        ...
        cols = ("Date", "Time", "Invoice", "Customer", "Phone", "Pay", "Total", "Paid", "Due")

        self.tree = ttk.Treeview(left_f, columns=cols,

                                  show="headings", height=20)

        widths = [100, 65, 125, 130, 110, 75, 90, 90, 90]
        ...
        for col, w in zip(cols, widths):
            ...
            self.tree.column(col, width=w, minwidth=min_widths.get(col, w))
        ...
        for r in page.rows:
            ...
            node = self.tree.insert(
                ...
                values=report_tree_values(
                    r,
                    display_date=iso_to_display_date,
                    currency=fmt_currency,
                    include_paid_due=True,
                ),
            )
    ...
    def _print_preview(self):
        ...
        try:
            ...
            messagebox.showwarning(
                ...
                "Direct printer module is missing, so text preview fallback was opened.\n\n"
            ...
            if any(token in detail for token in ("printer", "spool", "default printer", "win32print")):
                ...
                messagebox.showwarning(
                    "Print Fallback",
                    "Direct printer printing is not available, so text preview fallback was opened.\n\n"
                    f"Detail: {e}\n\n"
                    f"Print file: {path}"
                )
            else:
    ...
    def _set_report_filter_mode(self, mode: str):
        self._active_report_filter_mode = mode
        buttons = getattr(self, "_report_filter_buttons", {})
        colors = getattr(self, "_report_filter_button_colors", {})
        for key, btn in buttons.items():
            base = colors.get(key, "#636e72")
            active = C["green"] if key == mode else base
            try:
                btn.set_color(active, C["blue"])
            except Exception:
                pass
    ...
    def _set_today(self):
        ...
        self._set_report_filter_mode("today")
    ...
    def _set_month(self):
        ...
        self._set_report_filter_mode("month")
    ...
    def _set_all(self):
        ...
        self._set_report_filter_mode("all")
