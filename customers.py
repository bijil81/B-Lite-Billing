...
from ui_responsive import make_scrollable, make_toplevel_scrollable_with_footer, get_responsive_metrics, scaled_value, fit_toplevel
...
class CustomersFrame(tk.Frame):
    ...
    def _build(self):
        ...
        cols = ("Name", "Phone", "Due", "Visits", "Points",
    ...
    def _do_load(self):
        ...
        for ph, c in customers.items():
            ...
            due_val= float(c.get("current_due", 0.0) or 0.0)
            ...
            is_bl = bool(c.get("is_blacklisted", False))



            rows.append((name, ph, due_val, len(visits), pts,



                         round(spent, 2), last, is_bl))
        ...
        try:
            ...
            sort_idx = ["Name", "Phone", "Due", "Visits", "Points",
        ...
        self.tree.tag_configure('has_due', foreground=C.get("orange", "#e67e22"))



        self.tree.tag_configure('blacklisted', foreground="#ef4444")
        ...
        for r in rows[start:end]:
            ...
            display_name = ("[BL] " + r[0]) if r[7] else r[0]
            ...
            iid = self.tree.insert("", tk.END,
                              ...
                              values=(display_name, r[1], fmt_currency(r[2]), r[3], r[4],



                                      fmt_currency(r[5]), r[6]))
            ...
            if r[7]:
                ...
                self.tree.item(iid, tags=('blacklisted',))
                ...
                self.tree.item(iid, tags=('has_due',))
    ...
    def _show_load(self, text="Loading customers..."):
        ...
        try:
            ...
            app_log(f"[customers._show_load] {e}")
    ...
    def _hide_load(self):
        ...
        if hasattr(self, "_load_lbl") and self._load_lbl:
            ...
            try:
                ...
                app_log(f"[customers._hide_load] {e}")
    ...
    def _create_bill_from_context(self, context):
        ...
        try:
            ...
            try:
                ...
                app_log(f"[customers._create_bill_from_context] Refreshing billing frame failed: {e}")
    ...
    def _cust_form(self, title, phone="", name="",
        ...
        existing = get_customers().get(phone, {}) if edit_mode else {}



        limit_val = existing.get("credit_limit", 0.0)



        due_val = existing.get("current_due", 0.0)
        ...
        def _save():
            ...
            try:
                ...
                cl = float(limit_str) if limit_str else 0.0
            ...
            try:
                ...
                app_log(f"[customers._cust_form._save] Activity log failed: {e}")
        ...
        ModernButton(footer, text="Save Customer", image=get_action_icon("save"), compound="left",
        ...
        ModernButton(footer, text="Close", command=lambda: (win.grab_release(), win.destroy()),
                     color=C["sidebar"], hover_color=C["blue"],
                     width=scaled_value(110, 104, 96), height=scaled_value(38, 36, 32), radius=8,
                     font=("Arial", scaled_value(10, 10, 9), "bold"),
    ...
    def _cust_form_v5(self, title, phone="", name="", birthday="", edit_mode=False):
        ...
        fit_toplevel(
            ...
            scaled_value(520, 490, 460),



            scaled_value(500, 470, 440),
        ...
        for idx, (lbl, val) in enumerate(fields):
            ...
            if idx == 4 and edit_mode and due_val > 0:
                row_f = tk.Frame(f, bg=C["bg"])
                row_f.pack(fill=tk.X, pady=(3, 10))
                
                e = tk.Entry(row_f, font=("Arial", 11), bg=C["input"], fg=C["text"], bd=0, insertbackground=C["accent"])
                e.pack(side=tk.LEFT, fill=tk.X, expand=True, ipady=6, padx=(0, 10))
                e.insert(0, val)
                e.config(state="disabled")
                
                btn = tk.Button(
                    row_f, text="Settle Due", bg=C.get("green", "#2ecc71"), fg="white",
                    font=("Arial", 11, "bold"), bd=0, cursor="hand2",
                    activebackground=C.get("dark_green", "#27ae60"), activeforeground="white",
                    command=lambda e=existing, mw=win: self._show_settle_due_dialog(e, mw)
                )
                btn.pack(side=tk.RIGHT, ipady=4, ipadx=10)
            else:
        ...
        blacklist_var = tk.BooleanVar(value=bool(existing.get("is_blacklisted", False)))
        bl_row = tk.Frame(f, bg=C["bg"])
        bl_row.pack(anchor="w", pady=(4, 8))
        tk.Checkbutton(
            bl_row, text="Blacklisted (blocks credit billing)",
            variable=blacklist_var,
            bg=C["bg"], fg=C.get("orange", "#e67e22"),
        ...
        def _save():
            ...
            save_customer_v5({
                ...
                "is_blacklisted": blacklist_var.get(),
            ...
            try:
                ...
                app_log(f"[customers._cust_form_v5._save] Activity log failed: {e}")
        ...
        ModernButton(footer, text="Save Customer", image=get_action_icon("save"), compound="left", command=_save, color=C["teal"], hover_color=C["blue"], width=340, height=38, radius=8, font=("Arial", 11, "bold")).pack(side=tk.LEFT, fill=tk.X, expand=True)
        ModernButton(footer, text="Close", command=lambda: (win.grab_release(), win.destroy()), color=C["sidebar"], hover_color=C["blue"], width=110, height=38, radius=8, font=("Arial", 10, "bold")).pack(side=tk.RIGHT, padx=(10, 0))
    ...
    def refresh_deleted_customer_cache(self) -> None:
        ...
        try:
            ...
            app_log(f"[customers.refresh_deleted_customer_cache] Failed to invalidate cache: {e}")
        ...
        if messagebox.askyesno(
            ...
            try:
                ...
                try:
                    ...
                    app_log(f"[customers._delete] Activity log failed: {e}")
    ...
    def _resize_directory_columns(self, event=None):
        ...
        col_map = {
            ...
            "Name": max(120, int(width * 0.20)),



            "Phone": max(100, int(width * 0.15)),



            "Due": max(72, int(width * 0.10)),



            "Visits": max(60, int(width * 0.08)),



            "Points": max(60, int(width * 0.08)),



            "Total Spent": max(100, int(width * 0.15)),



            "Last Visit": max(110, width - (



                max(120, int(width * 0.20)) +



                max(100, int(width * 0.15)) +



                max(72, int(width * 0.10)) +
    ...
    def _schedule_history_resize(self, tree):
        ...
        if self._history_resize_job:
            ...
            try:
                ...
                app_log(f"[customers._schedule_history_resize] after_cancel failed: {e}")
...
# v5 compatibility overrides -------------------------------------------------
    ...
    def _show_settle_due_dialog(self, customer: dict, parent_win: tk.Toplevel):
        from tkinter import ttk, messagebox
        from services_v5.payment_receipt_service import generate_settlement_receipt
        import os

        settle_win = tk.Toplevel(parent_win)
        settle_win.title("Settle Due")
        settle_win.geometry("400x350")
        settle_win.configure(bg=C["bg"])
        settle_win.transient(parent_win)
        settle_win.grab_set()

        # Center dialog
        settle_win.update_idletasks()
        x = parent_win.winfo_x() + (parent_win.winfo_width() - 400) // 2
        y = parent_win.winfo_y() + (parent_win.winfo_height() - 350) // 2
        settle_win.geometry(f"+{x}+{y}")

        tk.Label(settle_win, text=f"Settle Due: {customer.get('name', 'Customer')}", 
                 font=("Arial", 14, "bold"), bg=C["bg"], fg=C["text"]).pack(pady=15)

        due_val = float(customer.get("current_due", 0.0) or 0.0)

        f = tk.Frame(settle_win, bg=C["bg"])
        f.pack(fill=tk.BOTH, expand=True, padx=30, pady=10)

        tk.Label(f, text=f"Current Due: ₹{due_val:.2f}", font=("Arial", 12), bg=C["bg"], fg=C["orange"]).pack(anchor="w", pady=(0, 15))

        tk.Label(f, text="Amount Paying (₹):", font=("Arial", 11), bg=C["bg"], fg=C["text"]).pack(anchor="w")
        amt_ent = tk.Entry(f, font=("Arial", 12), bg=C["input"], fg=C["text"], insertbackground=C["accent"])
        amt_ent.pack(fill=tk.X, ipady=5, pady=(5, 15))
        amt_ent.insert(0, str(due_val))

        tk.Label(f, text="Payment Mode:", font=("Arial", 11), bg=C["bg"], fg=C["text"]).pack(anchor="w")
        mode_var = tk.StringVar(value="Cash")
        modes = ["Cash", "Card", "UPI"]
        mode_combo = ttk.Combobox(f, textvariable=mode_var, values=modes, state="readonly", font=("Arial", 11))
        mode_combo.pack(fill=tk.X, ipady=5, pady=(5, 20))

        def _confirm_settlement():
            try:
                amt_str = amt_ent.get().strip()
                if not amt_str:
                    messagebox.showerror("Error", "Please enter amount", parent=settle_win)
                    return
                amt = float(amt_str)
            except ValueError:
                messagebox.showerror("Error", "Invalid amount", parent=settle_win)
                return

            try:
                username = self.app.current_user.get("username", "Admin") if getattr(self, "app", None) and hasattr(self.app, "current_user") else "Admin"
                from adapters.customer_adapter import _service as customer_service
                res = customer_service.settle_customer_due(
                    customer.get("legacy_phone") or customer.get("phone"),
                    amt,
                    mode_var.get(),
                    handled_by=username
                )
                ...
                # Offer receipt
                if messagebox.askyesno("Success", f"Due settled successfully.\nNew Due: ₹{res['new_due']:.2f}\n\nDo you want to generate a receipt?", parent=settle_win):
                    try:
                        from utils import BILLS_DIR, open_file_cross_platform
                        pdf_path = os.path.join(BILLS_DIR, f"{res['receipt_no']}.pdf")
                        generate_settlement_receipt(res, pdf_path)
                        open_file_cross_platform(pdf_path)
                    except Exception as pdf_err:
                        from utils import app_log
                        app_log(f"Receipt generation failed: {pdf_err}")
                        messagebox.showerror("Error", f"Failed to generate receipt: {pdf_err}", parent=settle_win)
                
                # Close the settle dialog and the parent edit dialog
                settle_win.destroy()
                parent_win.destroy()
                    
            except Exception as e:
                from utils import app_log
                app_log(f"Settlement failed: {e}")
                # Use self as parent just in case settle_win is somehow destroyed
                messagebox.showerror("Error", str(e))

        btn_f = tk.Frame(settle_win, bg=C["bg"])
        btn_f.pack(fill=tk.X, padx=30, pady=20)
        
        tk.Button(btn_f, text="Cancel", font=("Arial", 11), bg=C["card"], fg=C["text"], bd=0,
                  command=settle_win.destroy).pack(side=tk.LEFT, ipadx=15, ipady=5)
        tk.Button(btn_f, text="Confirm Payment", font=("Arial", 11, "bold"), bg=C.get("green", "#2ecc71"), fg="white", bd=0,
                  command=_confirm_settlement).pack(side=tk.RIGHT, ipadx=15, ipady=5)
